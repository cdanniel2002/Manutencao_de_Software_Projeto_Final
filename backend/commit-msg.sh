...
CONVENTIONAL_COMMIT_REGEX='^(feat|fix|docs|style|refactor|test|chore|build|ci|perf|revert|remove)(\([a-zA-Z0-9_.-]+\))?(!)?:\s.*$'
...
if ! [[ $COMMIT_MSG =~ $CONVENTIONAL_COMMIT_REGEX ]]; then
    ...
    echo "  remove:   Exclusão de arquivos, diretórios ou funcionalidades obsoletas ou não utilizadas."
    echo
    echo "Exemplos:"
